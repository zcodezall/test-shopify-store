...
class VariantSelects extends HTMLElement {
  ...
  toggleAddButton(disable = true, text, modifyClass = true) {
    ...
    const backorderNote = document.querySelector('.backorderNote');
    const backorderHtml = `Available on backorder`;
    const swatchLink = document.querySelector('.abc');
    const swatch = variantSwatch[this.currentVariant.id];
    const title = variantSwatch[this.currentVariant.id].title;
    const swatchItem = variantSwatch[this.currentVariant.id].swatchitem;
    console.log('helllolll', typeof title, title);
    if(variantSwatch[this.currentVariant.id] != false){
    var swatchLinkHtml = `<a href="#"
      onclick="addSwatchToCart('${title}', '${swatchItem}')"
      >
        Request a Swatch
    </a>`;
    }
      ...
      if (disable) {
        ...
        backorderNote.innerHTML = "";
        swatchLink.innerHTML = "";
      } else {        
        addButton.removeAttribute('disabled');
        addButtonText.textContent = window.variantStrings.addToCart;
        if(variantSwatch[this.currentVariant.id] != false){
          swatchLink.innerHTML = swatchLinkHtml;
        }
        else{
          swatchLink.innerHTML = "";
        }
        ...
        if(backorderStatus[this.currentVariant.id] == true){
          
          backorderNote.textContent = backorderHtml;
        }
        else{
          backorderNote.textContent = '';
        }
      }
